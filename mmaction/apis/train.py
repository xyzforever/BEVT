...
from ..core import (DistEvalHook, EvalHook, OmniSourceDistSamplerSeedHook,
                    OmniSourceRunner, MultiSourceRunner)
from ..datasets import build_dataloader, build_dataset
from ..utils import PreciseBNHook, get_root_logger
from .test import multi_gpu_test
from mmcv_custom.runner import EpochBasedRunnerAmp, MultiSourceRunnerAmp
import apex
import os.path as osp
...
def train_model(model,
    ...
    dataloader_setting = dict(
        ...
        dist=distributed,
        seed=cfg.seed,
        tsv_loader=cfg.data.get('tsv_loader', False)
    )
    ...
    if cfg.omnisource:
        ...
        data_type = cfg.data.get('data_type', ['video'] * len(dataset))
        update_interval = cfg.optimizer_config.get('update_interval', 1)
        omni_videos_per_gpu = cfg.data.get('omni_videos_per_gpu', None)
        omni_videos_per_gpu = [bz // update_interval for bz in omni_videos_per_gpu]
    ...
    if use_amp:
        Runner = MultiSourceRunnerAmp if cfg.omnisource else EpochBasedRunnerAmp
        runner = Runner(
            model,
            optimizer=optimizer,
        ...
        Runner = MultiSourceRunner if cfg.omnisource else EpochBasedRunnerAmp
        runner = Runner(
            model,
            optimizer=optimizer,
            work_dir=cfg.work_dir,
    ...
    runner_kwargs = dict()
    if cfg.omnisource:
        runner_kwargs = dict(train_ratio=train_ratio, data_type=data_type)

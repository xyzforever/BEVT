...
class FormatShapeByKey:
    ...
    def __init__(self, input_format, format_key, collapse=False):
    ...
    def __call__(self, results):
        ...
        Args:
            results (dict): The resulting dict to be modified and passed
                to the next transform in pipeline.
        """
        if not isinstance(results[self.format_key], np.ndarray):
            results[self.format_key] = np.array(results[self.format_key])
        imgs = results[self.format_key]
        # [M x H x W x C]

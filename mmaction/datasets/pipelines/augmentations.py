...
@PIPELINES.register_module()
class CopyResize:
    def __init__(self, new_keys, scale=None, interpolation='lanczos'):
    ...
    def __call__(self, results):
        ...
        ori_imgs = results['imgs']
        if self.scale is not None:
            ori_imgs = [
                mmcv.imresize(
                    img, self.scale, interpolation=self.interpolation)
                for img in ori_imgs
            ]
        n = len(ori_imgs)
        h, w, c = ori_imgs[0].shape
        imgs = np.empty((n, h, w, c), dtype=np.float32)
        for i, img in enumerate(ori_imgs):
            imgs[i] = img

        results[self.new_keys] = imgs
        return results


@PIPELINES.register_module()
class MapPixels:
    def __init__(self, keys, logit_laplace_eps=0.1):
        self.keys = keys
        self.logit_laplace_eps = logit_laplace_eps

    def __call__(self, results):
        for key in self.keys:
            # scale to [0, 1.0]
            img = torch.true_divide(results[key], 255)
            img = (1 - 2 * self.logit_laplace_eps) * img + self.logit_laplace_eps
            results[key] = img
...
class ColorJitter:
    ...
    def __init__(self, brightness=0.5, contrast=0.5, saturation=0.5, hue=0.1, frame_independent=False):
        ...
        self.frame_independent = frame_independent
